...
mock_context.resource = {
                         ...
                         'name': 'projects/my-gcp-project/topics/blob-update-storage-class', 
                         'type': 'type.googleapis.com/google.pubsub.v1.PubsubMessage'
}

CLIENT = storage.Client()

class TestMakeUniqueTestId:

    def test_expected(self):
        nodes = []
        datetime_stamp = ""

        task_id, trunc_nodes_hash = main.make_unique_task_id(nodes, datetime_stamp)
        assert task_id ==
        assert trunc_nodes_hash ==
    ...
    def test_expected(self):
        data = {
                'body': {
                         ...
                         'results': {
                                     'bucket': 'my-gcp-project-from-personalis', 
