...
def launch_extract_unmapped_reads(event, context):
    ...
    task_name = "extract-unmapped-reads"
    unique_task_label = "ExtractUnmappedReads"
    job_dict = {
             "provider": "google-v2",
             "user": DSUB_USER,
             "regions": REGIONS,
             ...
             "diskSize": 1000,
             "command": "samtools view -bh -f4 ${INPUT} --reference ${REF} > ${OUTPUT}",
             "envs": {
                    "SAMPLE_ID": sample
             },
             "inputs": {
             ...
             "outputs": {
                    "OUTPUT": f"gs://{OUT_BUCKET}/{plate}/{sample}/{task_name}/{task_id}/output/{sample}.ubam"
             },
