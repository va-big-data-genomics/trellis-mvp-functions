...
def launch_flagstat(event, context):
    ...
    dsub_args = [
        ...
        "--image", 
            f"gcr.io/{project_id}/samtools:1.01", 
        "--command", "samtools flagstat ${INPUT} > ${OUTPUT}", 
        "--input", 
            f"INPUT=gs://{in_bucket}/{in_path}",
        "--output", 
            f"OUTPUT=gs://{out_bucket}/{out_root}/{task_group}/{task_name}/objects/{basename}.flagstat.tsv",
        "--env", f"SAMPLE_ID={sample}_chromosome_{chromosome}"
    ]
