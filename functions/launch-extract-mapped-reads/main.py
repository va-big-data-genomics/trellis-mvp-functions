...
def launch_extract_mapped_reads(event, context):
    ...
    node = message.node
    region_key = message.results['region-key']

    if not node:
        print("> No node provided. Exiting.")
        return

    bed = parsed_vars[f"{region_key.upper()}_BED"]
    ...
    task_name = "extract-mapped-reads"
    unique_task_label = "ExtractMappedReads"
    job_dict = {
             "provider": "google-v2",
             "user": DSUB_USER,
             "regions": REGIONS,
             ...
             "diskSize": 1000,
             "command": "samtools view -bh ${INPUT} --reference ${REF} -L ${BED} > ${OUTPUT}",
             "envs": {
                    "SAMPLE_ID": sample
             },
             "inputs": {
             ...
             "outputs": {
                    "OUTPUT": f"gs://{OUT_BUCKET}/{plate}/{sample}/{task_name}/{task_id}/output/{sample}.{region_key}.bam"
             },
