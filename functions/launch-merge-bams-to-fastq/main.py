...
def launch_merge_bams_to_fastq(event, context):
    ...
    input_ids = []
    for node in nodes:
        if 'Bam' not in node['labels'] or not 'Ubam' in node['labels']:
            print(f"Error: inputs must be u/bams.")
            return
    ...
    task_name = "merge-bams-to-fastq"
    unique_task_label = "MergeBamsToFastq"
    job_dict = {
             "provider": "google-v2",
             "user": DSUB_USER,
             "regions": REGIONS,
             ...
             "diskSize": 1000,
             "command": "samtools merge ${MERGED_BAM} ${BAM1} ${BAM2}; samtools fastq ${MERGED_BAM} > ${MERGED_FASTQ}",
             "inputs": {
                    "BAM1": bams[0],
                    "BAM2": bams[1]
             },
             "outputs": {
                    "MERGED_BAM": f"gs://{OUT_BUCKET}/{plate}/{sample}/{task_name}/{task_id}/output/{sample}.merged.bam",
                    "MERGED_FASTQ": f"gs://{OUT_BUCKET}/{plate}/{sample}/{task_name}/{task_id}/output/{sample}.merged.fastq"
             },
