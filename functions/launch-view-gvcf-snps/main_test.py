...
with open('trellis-config.yaml') as fh:
    trellis_config = yaml.load(fh, Loader=yaml.Loader)
trellis = Struct(**trellis_config)
...
class TestCheckConditions:
    ...
    def test_expected(self):
        node = {'labels': ['WGS35', 'Blob', 'Cromwell', 'Gatk', 'Gzipped', 'Merged', 'Vcf']}

        result = main.check_conditions(node)
        assert result == True
...
class TestLaunchViewGvcfSnps:

    def test_expected(self):
        # Get data from local JSON
        with open('test-data.json', 'r') as fh:
            data = json.load(fh)

        data_utf8 = json.dumps(data).encode('utf-8')
        event = {'data': base64.b64encode(data_utf8)}

        message = main.launch_view_gvcf_snps(event, mock_context, test=True)