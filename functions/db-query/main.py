...
def main(event, context, local_driver=None):
    ...
    if query_request.custom == True:
        ...
        # Note: Removing this section because QUERY_DICT is only going to contain
        # user-defined queries and custom queries should never be found in them.
            ...
            for query in create_job_queries:
                if database_query.name == query.name:
                    if database_query == query:
                        logging.info(f"Merge job query {database_query.name} alread stored.")
                        register_new_query = False
                    else:
                        logging.error(f"Merge job query {database_query.name} is already stored in " +
